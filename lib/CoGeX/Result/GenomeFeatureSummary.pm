package CoGeX::Result::GenomeFeatureSummary;
...
information for a dataset group in the CoGe database.
...
__PACKAGE__->result_source_instance->view_definition(<<'END_OF_DEFINITION');
    ...
    FROM feature f
    JOIN feature_type t      ON t.feature_type_id = f.feature_type_id
    JOIN dataset_connector c ON c.dataset_id = f.dataset_id
    WHERE c.dataset_group_id = ?
